...
def get_hfi_returns():
    ...
    hfi = pd.read_csv(
        "resources/data/edhec-hedgefundindices.csv",
        header=0,
        index_col=0,
        parse_dates=True,
    )
...
def get_ind_returns():
    ...
    ind = pd.read_csv("resources/data/ind30_m_vw_rets.csv", header=0, index_col=0) / 100
...
def get_ind_size():
    ...
    ind = pd.read_csv("resources/data/ind30_m_size.csv", header=0, index_col=0)
    ind.index = pd.to_datetime(ind.index, format="%Y%m").to_period("M")
    ind.columns = ind.columns.str.strip()
...
def get_ind_nfirms():
    ...
    ind = pd.read_csv("resources/data/ind30_m_nfirms.csv", header=0, index_col=0)
    ind.index = pd.to_datetime(ind.index, format="%Y%m").to_period("M")
    ind.columns = ind.columns.str.strip()
...
def is_normal(r, level=0.01):
    """
    Applies the Jarque-Bera test to determine whether a Series is normal or not
...
def var_gaussian(r, level=5, modified=False):
    ...
    if modified:
        # modify the Z score based on observed skewness and kursosis
        s = skewness(r)
        k = kurtosis(r)
        z = (
            z
...
def annualize_rets(r, periods_per_year):
    ...
    buut that is currently left as an exercise
    to the reader
    """
...
def plot_ef2(n_points, er, cov, style=".-"):
    ...
    if er.shape[0] != 2 or cov.shape[0] != 2:
        raise ValueError("plot-ef2 can only plot 2-asset frontiers")
